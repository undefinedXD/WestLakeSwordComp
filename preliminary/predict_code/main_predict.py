...
def test_func(test_path,save_path):
    ...
    test_features_selected.drop(['appProtocol','tlsSubject','tlsIssuerDn','tlsSni_fromTor','temp','eventId'],inplace=True,axis=1)
    ...
    for i in range(5):
        model = joblib.load('../model/lgb_cv_{}.pkl'.format(i))
        target_enc = joblib.load('../model/encoder_cv_{}.pkl'.format(i))
        # 对高维类别变量进行编码
