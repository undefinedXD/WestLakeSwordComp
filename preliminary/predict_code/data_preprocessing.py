...
def feature_engineering(data):
    ...
    data['tlsSni_fromTor'] = data['tlsSni'].str.contains('onion|tor2web|4tor|d2web.org|civiclink.network') #https://www.cnblogs.com/royfans/p/12722792.html

    data['confusion'] = data[data.tlsSni.notna()].tlsSni.apply(lambda x:(len(split(x))-1)/len(x))
    data['confusion_CN'] = data[data.CN.notna()].CN.apply(lambda x:(len(split(x))-1)/len(x))
    ...
    data['CN_point_rate'] = data[data['CN'].notna()].CN.apply(lambda x: x.count('.') / len(x))
    data['CN_alp_rate'] = data[data['CN'].notna()].CN.apply(lambda x: len(re.findall('[a-zA-Z]', x)) / len(x))
    data['CN_num_rate'] = data[data['CN'].notna()].CN.apply(lambda x: len(re.findall('[0-9]', x)) / len(x))
